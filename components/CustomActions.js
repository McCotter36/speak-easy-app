...
export default class CustomActions extends React.Component {
  /** Let user choose an image from the device library */
  pickImage = async () => {
    // ask permission to access media library
    let { status } = await Permissions.askAsync(Permissions.MEDIA_LIBRARY);
    try{
    // if permission granted open image library with images as media type
      if (status === 'granted') {
        let result = await ImagePicker.launchImageLibraryAsync({
          mediaTypes: ImagePicker.MediaTypeOptions.Images,
        }).catch(error => console.log(error));

        // unless cancelled, send image to firebase storage
        if (!result.cancelled) {
  ...
  /** Let user take a photo with devices camera */
  takePhoto = async () => {
    ...
    try{
      if (status === 'granted') {
        ...
        //unless cancelled, send photo to firebase storage
        if (!result.cancelled) {
          const imageUrl = await this.uploadImageFetch(result.uri);
  ...
  /** Let user get their location using device GPS */
  getLocation = async () => {
    try {
  ...
  /** handles communication feature expand options */
  onActionPress = () => {
    const options = [
      'Choose From Library', 
      'Take Picture', 
